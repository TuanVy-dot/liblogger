...
LOGGER *logger_create(const char *ref, FILE *file, const char *format, const log_level_t level) {
 ...
 * where each element getting print using print_element function
 * which should have accept arguement as pointer to the value getting print
 * the msg will get print first using __logger_msg__
 * Now the format is fixed for simplicity
 * 0: 1, 1: 2, 3: 2, i: array[i] 
 * Customization might come */
...
/* Take a string and parse it into an easier to use linked list */
static Node *logger_formatter(const char *s) {
    if (!s) return NULL;
